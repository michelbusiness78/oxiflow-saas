  ...
  const columns: Column<ContratWithClient>[] = [
    {
      ...
      cell: (r) => (
        <div>
          <p className="font-semibold text-slate-700">{r.client_nom ?? '—'}</p>
    ...
    },
    {
      ...
      cell: (r) => r.montant_mensuel ? (
        <div>
          <p className="font-semibold text-slate-800">{fmtEur(r.montant_mensuel)}<span className="text-xs text-slate-400"> /mois</span></p>
          <p className="text-xs text-slate-400">{fmtEur(r.montant_mensuel * 12)} /an</p>
        </div>
      ...
      cell: (r) => (
        <button
          ...
          className={[
            ...
            r.actif ? 'bg-blue-600' : 'bg-slate-200',
          ].join(' ')}
          role="switch"
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-slate-500">{contrats.length} contrat{contrats.length !== 1 ? 's' : ''}</p>
        <button
